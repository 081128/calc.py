...
def init_db():
    with sqlite3.connect(DB_NAME, check_same_thread=False) as conn:
        ...
        cursor.execute('''CREATE TABLE IF NOT EXISTS ventas (
            ...
            p_venta REAL, total REAL, estado TEXT)''')
        cursor.execute('''CREATE TABLE IF NOT EXISTS apartados (
            id TEXT, cliente TEXT, fecha TEXT, producto TEXT, 
            modelo TEXT, color TEXT, talla TEXT, cantidad INTEGER, estado TEXT)''')
        conn.commit()
                ...
                if st.button(f"✅ Finalizar e Imprimir (${total_v:,.2f})", type="primary", use_container_width=True):
                    ...
                    for i in st.session_state.carrito:
                        run_query("INSERT INTO ventas VALUES (?,?,?,?,?,?,?,?,?,?,?)", 
                                 (t_id, now.strftime("%Y-%m-%d"), now.strftime("%H:%M"), 
                                  i['producto'], i['modelo'], i['color'], i['talla'], 
                                  i['cantidad'], i['precio'], i['subtotal'], "COMPLETADA"))
                        run_query("UPDATE inventario SET stock = stock - ? WHERE modelo=? AND color=? AND talla=?", 
                                 (i['cantidad'], i['modelo'], i['color'], i['talla']))
        ...
        with st.form("ap_f"):
            ...
            if st.form_submit_button("Guardar Apartado"):
                ...
                run_query("INSERT INTO apartados VALUES (?,?,?,?,?,?,?,?,?)", 
                         (ap_id, cli, datetime.now().strftime("%Y-%m-%d"), r['producto'], 
                          r['modelo'], r['color'], r['talla'], cnt, "ACTIVO"))
                run_query("UPDATE inventario SET stock = stock - ? WHERE modelo=? AND color=? AND talla=?", 
                         (cnt, r['modelo'], r['color'], r['talla']))
                
                st.session_state.ticket_a_imprimir = generar_ticket_html("VALE APARTADO", ap_id, 
                    [{'modelo': r['modelo'], 'cantidad': cnt, 'subtotal': r['p_venta']*cnt}], 
                    r['p_venta']*cnt, cliente=cli)
                st.rerun()
    else:
        st.info("No hay stock para realizar apartados.")
...
# --- 4. CORTE DE CAJA ---
    ...
    if not df_corte.empty:
        ...
        m3.metric("Utilidad Neta", f"${utilidad:,.2f}", 
                  delta=f"{(utilidad/total_ventas)*100:.1f}%" if total_ventas > 0 else "0%")
        
        st.subheader("Detalle de Ventas")
        st.dataframe(df_corte[['transaccion_id', 'fecha', 'modelo', 'talla', 'cantidad', 'total']], use_container_width=True)
    else:
...
# --- 6. ADMIN ---
    ...
    with st.form("adm"):
        ...
        p = c1.text_input("Artículo (ej. Pantalón)")
        m = c2.text_input("Modelo")
        col = c3.text_input("Color")
        t = c4.text_input("Talla")
        
        c5, c6, c7 = st.columns(3)
        s = c5.number_input("Stock Inicial", 0)
        pc = c6.number_input("Precio Compra", 0.0)
        pv = c7.number_input("Precio Venta", 0.0)
        
        if st.form_submit_button("Guardar en Inventario"):
            if p and m:
                run_query("INSERT OR REPLACE INTO inventario VALUES (?,?,?,?,?,?,?,?)", 
                         (p, m, col, t, s, pc, pv, ""))
                st.success(f"Producto {m} guardado/actualizado.")
                st.rerun()
            else:
                st.error("Artículo y Modelo son obligatorios.")

# --- 7. RESPALDOS ---
elif choice == "💾 Respaldos":
    st.header("Respaldos de Seguridad")
    if os.path.exists(DB_NAME):
        with open(DB_NAME, "rb") as f:
            st.download_button("📥 Descargar Base de Datos", f, f"Backup_Ilusion_{datetime.now().strftime('%Y%m%d')}.db")
        
        file_up = st.file_uploader("Restaurar desde un respaldo (.db)", type=["db"])
        if file_up and st.button("🚀 Confirmar Restauración"):
